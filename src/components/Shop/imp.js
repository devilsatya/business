...
const CreateProduct = () => {
  ...
  const { success, error, loading } = useSelector((state) => state.products); // Added loading state
  const navigate = useNavigate();
    ...
    files.forEach((file) => {
      ...
      reader.onload = () => {
        if (reader.readyState === 2) {
          setImages((prevImages) => [...prevImages, reader.result]); // Append new images
        }
      };
  ...
  const handleCategoryChange = (selectedCategory) => {
    ...
    setSubCategory(""); // Reset dependent dropdowns
    setSubSubCategory("");
  };

  const handleSubCategoryChange = (selectedSubCategory) => {
    setSubCategory(selectedSubCategory);
    const subcategoryData = filteredSubcategories.find(sub => sub.title === selectedSubCategory);
    setFilteredSubSubcategories(subcategoryData ? subcategoryData.subsubcategories : []); 
    setSubSubCategory(""); // Reset sub-subcategory dropdown
  };

  const handleImageDelete = (imageToDelete) => {
    setImages(images.filter((image) => image !== imageToDelete)); // Remove the selected image
  };
  ...
  const handleSubmit = (e) => {
    ...
    newForm.append("stockXS", stockXS);
    newForm.append("stockS", stockS);
    newForm.append("stockM", stockM);
    newForm.append("stockL", stockL);
    newForm.append("stockXL", stockXL);
    newForm.append("stockXXL", stockXXL);
  ...
  return (
    <div className="w-[90%] 800px:w-[50%] bg-white shadow h-[80vh] rounded-[4px] p-3 overflow-y-scroll">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          >
            ...
            {filteredSubSubcategories.map((subsub) => (
              <option value={subsub} key={subsub}>
                {subsub}
              </option>
            ))}
          </select>
        ...
        <br />
        {/* Stock Fields for Different Sizes */}
        <div>
          <label className="pb-2">Stock for ExtraSmall (XS)</label>
          <input
            type="number"
            value={stockXS}
        ...
        <div>
          <label className="pb-2">Stock for Small (S)</label>
          <input
            type="number"
            value={stockS}
        ...
        <div>
          <label className="pb-2">Stock for Medium (M)</label>
          <input
            type="number"
            value={stockM}
            onChange={(e) => setStockM(e.target.value)}
            className="mt-2 block w-full px-3 h-[35px] border border-gray-300 rounded-[3px]"
            placeholder="Enter stock for medium size..."
          />
        </div>
        <div>
          <label className="pb-2">Stock for Large (L)</label>
          <input
            type="number"
            value={stockL}
        ...
        <div>
          <label className="pb-2">Stock for XL</label>
          <input
            type="number"
            value={stockXL}
            onChange={(e) => setStockXL(e.target.value)}
            className="mt-2 block w-full px-3 h-[35px] border border-gray-300 rounded-[3px]"
            placeholder="Enter stock for XL size..."
          />
        </div>
        <div>
          <label className="pb-2">Stock for XXL</label>
          <input
            type="number"
            value={stockXXL}
            onChange={(e) => setStockXXL(e.target.value)}
            className="mt-2 block w-full px-3 h-[35px] border border-gray-300 rounded-[3px]"
            placeholder="Enter stock for XXL size..."
          />
        </div>
        <br />
...
const CreateEvent = () => {
  ...
  const handleImageChange = (e) => {
    ...
    // If more than 5 images are selected, prevent adding more
    ...
    <div className="w-[90%] 800px:w-[50%] bg-white  shadow h-[80vh] rounded-[4px] p-3 overflow-y-scroll">
      ...
      <form onSubmit={handleSubmit}>
              ...
              images.map((i, index) => (
                <div key={index} className="relative">
                  <img
                    src={i}
                    alt=""
                    className="w-[150px] h-[150px] m-2 border"
                  />
                  <button
        ...
        <div className="text-center">
          <button
            type="submit"
            className="bg-[#1890ff] px-6 py-2 rounded-lg text-white"
          >
            Create Event
          </button>
        </div>
