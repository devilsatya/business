...
const OrderDetails = () => {
  ...
  data.cart.map((item, index) => (
    <div className="w-full flex items-start mb-5" key={index}>
      <img
        src={`${item.images[0]?.url}`} // Assuming the first image in the array
        alt={item.name}
      ...
      <div className="w-full">
        ...
        {/* Iterate over the variations for each item */}
  ...
  <div className="w-full 800px:w-[60%]">
    ...
    <h4 className="pt-3 text-[20px]">
      {data?.shippingAddress?.houseNumber} {data?.shippingAddress?.street}
    </h4>
    <h4 className="text-[20px]">{data?.shippingAddress?.city}</h4>
    <h4 className="text-[20px]">{data?.shippingAddress?.state}</h4>
    <h4 className="text-[20px]">{data?.shippingAddress?.zipCode}</h4>
